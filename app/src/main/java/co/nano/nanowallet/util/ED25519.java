...
public class ED25519 {
    ...
    static final BigInteger qm2 = new BigInteger(
            "57896044618658097711785492504343953926634992332820282019728792003956564819947");
    static final BigInteger qp3 = new BigInteger(
    ...
    static final BigInteger I = new BigInteger(
            "19681161376707505956807079304988542015446066515923890162744021073123829784752");
    static final BigInteger By = new BigInteger(
            "46316835694926478169428394003475163141307993866256225615783033603165251855960");
    static final BigInteger Bx = new BigInteger(
            "15112221349535400772501151409588531511454012693041857206046113283949847762202");
    static final BigInteger[] B = { Bx.mod(q), By.mod(q) };
    static final BigInteger un = new BigInteger(
    ...
    static BigInteger[] scalarmult(BigInteger[] P, BigInteger e) {
        ...
        Q = edwards(Q, Q);
        // System.out.println("scalarmult aeQ = " + Q[0] + "," + Q[1] + " e="+e+"
        // testbit="+(e.testBit(0)?1:0));
        if (e.testBit(0))
            Q = edwards(Q, P);
    ...
    static boolean isoncurve(BigInteger[] P) {
        ...
        // System.out.println("isoncurve close with
        // "+xx.negate().add(yy).subtract(BigInteger.ONE).subtract(dxxyy).mod(q));
    ...
    static BigInteger[] decodepoint(byte[] s) throws Exception {
        ...
        // System.out.println("decodepoint open with s="+test.getHex(s)+"
        // ybyte="+test.getHex(ybyte));
        BigInteger y = new BigInteger(ybyte).and(un);
        // System.out.println("decodepoint y="+y);
        BigInteger x = xrecover(y);
        // System.out.println("decodepoint x="+x+" testbit="+(x.testBit(0)?1:0)+"
        // bit="+bit(s, b-1));
